...
public class NewSale implements ActionListener {
	
	private JButton dodaj, wyczysc, anuluj;

	private JTextField Nazwa, ilosc, koszt, data1, data2;
	
	private JFrame frame;

	private Jack link;
	
	public NewSale(Jack link) {
		
		this.link = link;
		
		frame = new JFrame();
		
		frame.setTitle("Dodaj zamówienie");
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(source == anuluj) {
			...
			try {
				link.dodajZamowienie(nazw, ilo, kosz, dat1, dat2);
				Sale.tab = link.selectZZamowien();
				Sale.mtblCalendar.addRow(Sale.tab[Sale.tab.length - 1]);	
